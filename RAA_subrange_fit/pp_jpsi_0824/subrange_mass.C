...
const char *kDataRoot = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/skimmedFiles/OniaRooDataSet_isMC0_JPsi_pp_y0.00_2.40_Effw0_Accw0_PtW1_TnP1_260303_root634.root";
...
// to read parameters from input files
...
// remove bad events to avoid nan error
double finite_or(double value, double fallback)
{
	return std::isfinite(value) ? value : fallback;
}

// ignore +-0.01% edges of data for stablility of fit and to avoid distortion from fluctuation of edges
...
// fit one mass distribution inside a single ctau subrange
	...
	bool drawMassSlices)
{
	...
	RooRealVar Nsig("Nsig", "Nsig", std::max(1.0, 0.35 * entries), 0.0, entries);
	RooRealVar Nbkg("Nbkg", "Nbkg", std::max(1.0, 0.65 * entries), 0.0, entries);
...
void subrange_mass(float ptLow = 3.0, float ptHigh = 6.5, float yLow = 1.6, float yHigh = 2.4,
	...
	bool applyQuantileCut = true, bool reuseMassSlices = false,
			...
			for (size_t batchStart = 0; batchStart + 1 < edges.size(); batchStart += static_cast<size_t>(jobs))
			{
				...
				for (size_t i = batchStart; i < batchEnd; ++i)
				{
					...
					const TString expr = TString::Format(
						"subrange_mass(%g,%g,%g,%g,%.17g,%.17g,%.17g,-1,%s,%s,%s,false,%d,1,false,%d,true)",
