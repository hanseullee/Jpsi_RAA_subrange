...
#   Refit final ctau only after changing manual ctau controls:
...
echo "[INFO] subrange controls: reuse_mass_slices=${reuse_mass_slices}, first_slice=${first_slice}, n_slices=${n_slices}, parallel=${parallel_mass_slices}, jobs=${max_parallel_jobs}, mass_slices_only=${mass_slices_only}"
...
bin_groups=(
  # "1.6 2.4|3.5 40.0; 3.5 6.5;6.5 9.0;9.0 12.0;12.0 40.0"
  # "1.6 2.4|3.5 40"
  
  # "0.0 1.6|6.5 40.0;6.5 9.0;9.0 12.0;12.0 15.0;15.0 20.0;20 25;25.0 40.0"
  # "0.0 1.6|6.5 40"
  "1.8 2.4|3.0 4.5; 4.5 5.5; 5.5 6.5; 6.5 7.5; 7.5 8.5; 8.5 9.5; 9.5 11.0; 11.0 15.0; 15.0 20.0; 20.0 40.0; 3.0 6.5; 6.5 40.0"
  "0.0 0.6|6.5 40.0;6.5 8.5;8.5 9.5;9.5 11.0;11.0 15.0;15 20;20.0 40.0"
  "0.0 0.6|20 40"
)
...
run_bin() {
    ...
    if [[ -n "$subrange_slice_list" ]]; then
      ...
      while IFS= read -r slice; do
        ...
        root_call "subrange_mass.C" \
          "subrange_mass(${pt_low},${pt_high},${y_low},${y_high},${ct_min},${ct_max},${ct_step},${max_slices},${draw_mass_slices},${trim_sparse_fwd},${apply_quantile_cut},false,${slice},1,false,${max_parallel_jobs},true)" \
          "$bin_log_file"
      done < <(expand_slice_list "$subrange_slice_list")
    else
      root_call "subrange_mass.C" \
        "subrange_mass(${pt_low},${pt_high},${y_low},${y_high},${ct_min},${ct_max},${ct_step},${max_slices},${draw_mass_slices},${trim_sparse_fwd},${apply_quantile_cut},false,${first_slice},${n_slices},${parallel_mass_slices},${max_parallel_jobs},${mass_slices_only})" \
        "$bin_log_file"
    fi
  fi
  ...
  if [[ "$run_subrange_ctau" == true ]]; then
    root_call "subrange_ctau.C" \
      "subrange_ctau(${pt_low},${pt_high},${y_low},${y_high},${ct_min},${ct_max},${ct_step},${max_slices},${draw_mass_slices},${trim_sparse_fwd},${apply_quantile_cut},true,${first_slice},${n_slices},${parallel_mass_slices},${max_parallel_jobs},false)" \
      "$bin_log_file"
  fi
...
summary_file="${log_dir}/summary.log"
{
  ...
  echo "[INFO] reuse_mass_slices=${reuse_mass_slices} first_slice=${first_slice} n_slices=${n_slices} parallel_mass_slices=${parallel_mass_slices} max_parallel_jobs=${max_parallel_jobs} mass_slices_only=${mass_slices_only}"
