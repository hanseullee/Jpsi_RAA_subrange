...
const char *kDataRoot = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/OniaRooDataSet_isMC0_JPsi_pp_y0.00_2.40_Effw0_Accw0_PtW1_TnP1_260303_root634.root";
const char *kDatasetName = "dataset";
const char *kReferenceDir = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/RAA_subrange_fit/pp_jpsi";
...
void subrange_ctau(float ptLow = 3.0, float ptHigh = 6.5, float yLow = 1.6, float yHigh = 2.4,
	...
	bool applyQuantileCut = true, bool reuseMassSlices = true,
	...
	bool massSlicesOnly = false)
{
	...
	// ctauFrac1/2/3 from ctau_pr.C are recursive coefficients. Convert them here
	// to the physical RooAddModel coefficients for components 2 and 3.
	const double ctauRecFrac1Val = useReferenceCtauMC ? read_double(referencePr.get(), "ctauFrac1", 0.5) : 0.5;
	const double ctauRecFrac2Val = useReferenceCtauMC ? read_double(referencePr.get(), "ctauFrac2", 0.2) : 0.2;
	const double ctauRecFrac3Val = useReferenceCtauMC ? read_double(referencePr.get(), "ctauFrac3", 0.1) : 0.1;
	RooRealVar ctauRecFrac1("ctauFrac1", "ctauFrac1", std::clamp(ctauRecFrac1Val, 1e-6, 0.999999), 1e-6, 0.999999);
	RooRealVar ctauRecFrac2("ctauFrac2", "ctauFrac2", std::clamp(ctauRecFrac2Val, 1e-6, 0.999999), 1e-6, 0.999999);
	RooRealVar ctauRecFrac3("ctauFrac3", "ctauFrac3", std::clamp(ctauRecFrac3Val, 1e-6, 0.999999), 1e-6, 0.999999);
	RooFormulaVar ctauCompFrac2Model("ctauCompFrac2Model", "(1-@0)*@1", RooArgList(ctauRecFrac1, ctauRecFrac2));
	RooFormulaVar ctauCompFrac3Model("ctauCompFrac3Model", "(1-@0)*(1-@1)*@2", RooArgList(ctauRecFrac1, ctauRecFrac2, ctauRecFrac3));
	...
	if (useReferenceCtauMC && nResolutionComponents >= 2)
	{
		...
		resolutionFracList.add(ctauRecFrac1);
		if (nResolutionComponents >= 3)
		{
			resolutionPdfList.add(ctauTime3);
			resolutionFracList.add(ctauCompFrac2Model);
		}
		if (nResolutionComponents >= 4)
		{
			resolutionPdfList.add(ctauTime4);
			resolutionFracList.add(ctauCompFrac3Model);
		}
	...
	if (useReferenceCtauMC)
	{
		...
		if (nResolutionComponents >= 2)
		{
			...
			addCtauConstraint("ctauFrac1", ctauRecFrac1, std::clamp(ctauRecFrac1Val, 1e-6, 0.999999), 0.05);
		}
		if (nResolutionComponents >= 3)
		{
			...
			addCtauConstraint("ctauFrac2", ctauRecFrac2, std::clamp(ctauRecFrac2Val, 1e-6, 0.999999), 0.05);
		}
		if (nResolutionComponents >= 4)
		{
			...
			addCtauConstraint("ctauFrac3", ctauRecFrac3, std::clamp(ctauRecFrac3Val, 1e-6, 0.999999), 0.05);
		}
	}
	...
	// ------------------------------------------------------------------
	// final ctau fit model and fraction parameter
	// ------------------------------------------------------------------
	const double totalYield = std::max(1.0, hSignalYield->Integral());
	RooRealVar bFraction("bFraction", "bFraction", 0.20, 0.0, 1.0);
	RooAddPdf ctauModel("ctauModel", "ctauModel", RooArgList(*signalNpTime, *promptTimePdf), RooArgList(bFraction));
	...
	if (ctauFitSlices.size() >= 2 && totalYield > 0.0)
	{
		...
		{
			std::cout << Form("[INFO] final ctau fit uses direct bFraction fitTo with IntegrateBins, SumW2Error(false), and %d ExternalConstraints.",
				ctauConstraints.getSize()) << std::endl;
		}
		else
		{
			std::cout << "[INFO] final ctau fit uses direct bFraction fitTo with IntegrateBins and SumW2Error(false)." << std::endl;
		}
		auto runCtauFit = [&](int strategy) -> RooFitResult * {
			if (applyCtauConstraints)
			{
				return ctauModel.fitTo(signalYieldData,
					Save(), IntegrateBins(1e-6), SumW2Error(false), Offset(true),
	...
	const double bFractionVal = bFraction.getVal();
	const double bFractionErr = bFraction.getError();
	const double promptYieldVal = totalYield * (1.0 - bFractionVal);
	const double nonpromptYieldVal = totalYield * bFractionVal;
	const double promptYieldErr = totalYield * bFractionErr;
	const double nonpromptYieldErr = totalYield * bFractionErr;
	{
		TLatex tx;
		tx.SetNDC();
	...
	}
	{
		...
		tp.DrawLatex(xtext, y0 + dy * k++, Form("N_{PR} = %.4g", promptYieldVal));
		tp.DrawLatex(xtext, y0 + dy * k++, Form("N_{NP} = %.4g", nonpromptYieldVal));
	}
	{
		TLatex tx;
		tx.SetNDC();
	...
	ctPullPlot->SetTitle("");
	RooHist *ctPull = ctFrame->pullHist("yieldData", "ctauTotal");
	if (ctPull)
	...
	if (!outFile.IsZombie())
	{
		...
		TParameter<int>("ctauFitUsesDirectBFraction", 1).Write();
		...
		TParameter<double>("Nprompt", promptYieldVal).Write();
		TParameter<double>("NpromptErr", promptYieldErr).Write();
		TParameter<double>("Nnonprompt", nonpromptYieldVal).Write();
		TParameter<double>("NnonpromptErr", nonpromptYieldErr).Write();
