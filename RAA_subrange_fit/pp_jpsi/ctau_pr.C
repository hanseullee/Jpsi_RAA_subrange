...
void ctau_pr(float ptLow = 6.5, float ptHigh = 9.0, float yLow = 0.0, float yHigh = 1.6,
	bool drawFromSavedFit = false, bool publish = false)
{
	...
	bool isWeight = false; // use unweighted signal MC
	// ------------------------------------------------------------------
	// model control
	// ------------------------------------------------------------------
	int nResolutionComponents = 4; // 1~4 components
	
	if (yLow == 1.6f)
	{
		// default for splitted ctau bins
		nResolutionComponents = 3;

		if (ptLow == 3.5f && ptHigh == 40.0f)
			nResolutionComponents = 4;
	}
	else if (yLow == 1.8f)
	{
		// default for splitted ctau bins
		...
		if (ptLow == 3.f && ptHigh == 6.5f)
			nResolutionComponents = 4;
		else if (ptLow == 6.5f && ptHigh == 40.0f)
			nResolutionComponents = 4;
	}
	else if (yLow == 0.0f)
	{
	...
	// load dataset
	// ------------------------------------------------------------------
	const char *DATA_ROOT = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/skimmedFiles/OniaRooDataSet_isMC1_Jpsi_pp_y0.00_2.40_Effw0_Accw0_PtW0_TnP0_230717.root";
	...
	// Keep ordered Gaussian widths through positive scale offsets.
	...
	RooRealVar ctauTime1Mean("ctauTime1Mean", "ctauTime1Mean", 0.0); // fix
	...
	RooRealVar ctauTime2Delta("ctauTime2Delta", "ctauTime2Delta", 0.020, 0.005, 0.30);
	...
	RooRealVar ctauTime3Delta("ctauTime3Delta", "ctauTime3Delta", 0.060, 0.005, 0.45);
	...
	RooRealVar ctauTime4Delta("ctauTime4Delta", "ctauTime4Delta", 0.35, 0.02, 1.50);
	...
	// ------------------------------------------------------------------
	RooRealVar ctauFrac1Var("ctauFrac1Var", "ctauFrac1Var", 0.5, 0.01, 0.98);
	RooRealVar ctauFrac2Var("ctauFrac2Var", "ctauFrac2Var", 0.5, 0.01, 0.98);
	RooRealVar ctauFrac3Var("ctauFrac3Var", "ctauFrac3Var", 0.5, 0.01, 0.95);

	// THE LIFETIME FIT
	// Fit the prompt resolution as a normalized mixture with recursive fractions.
	// The saved ctauFrac1/2/3 values are recursive coefficients; compFrac* are the
	// corresponding physical component fractions used for plotting and downstream checks.
	...
	const bool useRecursiveFractions = true;
	RooAddPdf time_pdf("time_pdf", "time_pdf", promptTimePdfList, promptTimeFracList, useRecursiveFractions);
	if (drawFromSavedFit)
	{
	...
	if (!drawFromSavedFit)
	{
		std::cout << std::endl << "********* Starting Prompt Ctau Fit **************" << std::endl << std::endl;
		timeResultHolder.reset(time_pdf.fitTo(*data, Save(), Strategy(1), PrintLevel(0), NumCPU(nCPU), SumW2Error(isWeight), Minimizer("Minuit2", "migrad"), Offset(true)));
		if (!timeResultHolder || timeResultHolder->status() != 0 || timeResultHolder->covQual() < 2)
		{
			const int status = timeResultHolder ? timeResultHolder->status() : -999;
			const int covQual = timeResultHolder ? timeResultHolder->covQual() : -999;
			std::cout << "[Retry] prompt ctau fit status=" << status << " covQual=" << covQual << "; retry default minimizer" << std::endl;
			timeResultHolder.reset(time_pdf.fitTo(*data, Save(), Strategy(2), PrintLevel(0), NumCPU(nCPU), SumW2Error(isWeight), Minimizer("Minuit2", "migrad"), Offset(true)));
		}
		time_result = timeResultHolder.get();
		std::cout << std::endl << "********* Finished Prompt Ctau Fit **************" << std::endl << std::endl;
		if (time_result)
			time_result->Print();
	}
	else
	...
	const double ctauRecFrac1 = (nResolutionComponents >= 2) ? ctauFrac1Var.getVal() : 1.0;
	const double ctauRecFrac2 = (nResolutionComponents >= 3) ? ctauFrac2Var.getVal() : 0.0;
	const double ctauRecFrac3 = (nResolutionComponents >= 4) ? ctauFrac3Var.getVal() : 0.0;
	const double compFrac1 = (nResolutionComponents == 1) ? 1.0 : ctauRecFrac1;
	const double compFrac2 = (nResolutionComponents >= 2) ? ((nResolutionComponents == 2) ? (1.0 - ctauRecFrac1) : (1.0 - ctauRecFrac1) * ctauRecFrac2) : 0.0;
	const double compFrac3 = (nResolutionComponents >= 3) ? ((nResolutionComponents == 3) ? (1.0 - ctauRecFrac1) * (1.0 - ctauRecFrac2) : (1.0 - ctauRecFrac1) * (1.0 - ctauRecFrac2) * ctauRecFrac3) : 0.0;
	const double compFrac4 = (nResolutionComponents >= 4) ? ((1.0 - ctauRecFrac1) * (1.0 - ctauRecFrac2) * (1.0 - ctauRecFrac3)) : 0.0;
	...
	if (!drawFromSavedFit)
	{
		...
		if (!resolutionFile.IsZombie())
		{
			...
			if (nResolutionComponents >= 2)
			{
				...
				TParameter<double>("ctauFrac1", ctauRecFrac1).Write();
			}
			if (nResolutionComponents >= 3)
			{
				TParameter<double>("ctauTime3Mean", ctauTime3Mean.getVal()).Write();
				TParameter<double>("ctauTime3Scale", ctauTime3Scale.getVal()).Write();
				TParameter<double>("ctauFrac2", ctauRecFrac2).Write();
			}
			if (nResolutionComponents >= 4)
			{
				TParameter<double>("ctauTime4Mean", ctauTime4Mean.getVal()).Write();
				TParameter<double>("ctauTime4Scale", ctauTime4Scale.getVal()).Write();
				TParameter<double>("ctauFrac3", ctauRecFrac3).Write();
			}
			TParameter<double>("ctauCompFrac1", compFrac1).Write();
			if (nResolutionComponents >= 2)
				TParameter<double>("ctauCompFrac2", compFrac2).Write();
			if (nResolutionComponents >= 3)
				TParameter<double>("ctauCompFrac3", compFrac3).Write();
			if (nResolutionComponents >= 4)
				TParameter<double>("ctauCompFrac4", compFrac4).Write();
			if (time_result)
	...
	if (!publish)
	{
		...
		if (nResolutionComponents >= 2)
			printVar("r_{1}", ctauFrac1Var);
		if (nResolutionComponents >= 3)
			printVar("r_{2}", ctauFrac2Var);
		printVar("mean", ctauTime1Mean);
