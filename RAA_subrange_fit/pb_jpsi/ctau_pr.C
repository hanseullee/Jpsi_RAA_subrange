...
void ctau_pr(float ptLow = 6.5, float ptHigh = 9.0, float yLow = 0.0, float yHigh = 1.6,
	bool drawFromSavedFit = false, bool publish = false, int cLow = 0, int cHigh = 180)
{
	...
	// load dataset
	// ------------------------------------------------------------------
	const char *DATA_ROOT = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/skimmedFiles/OniaRooDataSet_miniAOD_isMC1_JPsi_Prompt_cent0_200_Effw0_Accw0_PtW0_TnP0_240530.root";
	//const char *DATA_ROOT = "/data/Oniatree/skimmedFiles/OniaRooDataSet_miniAOD_isMC1_JPsi_Prompt_cent0_200_Effw0_Accw0_PtW0_TnP0_240530.root";
	...
	// ------------------------------------------------------------------
	const auto ctRange = quantileRange(*dataBasic, *timeTmp, 0.0005, 0.9995, false);
	...
	RooRealVar ctauTime3Delta("ctauTime3Delta", "ctauTime3Delta", 0.060, 0.001, 0.45);
	...
	if (!drawFromSavedFit)
	{
		...
		timeResultHolder.reset(time_pdf.fitTo(*data, Save(), Strategy(2), PrintLevel(0), NumCPU(nCPU), SumW2Error(isWeight), Minimizer("Minuit2", "migrad")));
		if (!timeResultHolder || timeResultHolder->status() != 0 || timeResultHolder->covQual() < 2)
		{
			...
			timeResultHolder.reset(time_pdf.fitTo(*data, Save(), Strategy(2), PrintLevel(0), NumCPU(nCPU), SumW2Error(isWeight)));
		}
