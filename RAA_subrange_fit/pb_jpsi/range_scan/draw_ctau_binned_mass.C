/*
 * Draw dimuon mass spectra in ctau3D slices for a selected PbPb J/psi
 * kinematic bin.
 *
 * Workflow:
 *   - Load the PbPb J/psi RooDataSet from kDataRoot.
...
//const char *kDataRoot = "/data/users/pjgwak/work/raa_pb18/run2_raa_pbpb2018/skimmedFiles/OniaRooDataSet_miniAOD_isMC0_JPsi_cent0_200_Effw0_Accw0_PtW0_TnP0_230721.root";
...
TString bin_tag(double ptLow, double ptHigh, double yLow, double yHigh, int cLow, int cHigh)
{
	return TString::Format("c%d_%d_%s_%s", cLow, cHigh, y_tag(yLow, yHigh).Data(), pt_tag(ptLow, ptHigh).Data());
}

TString centrality_path(int cLow, int cHigh)
{
	return (cLow == 0 && cHigh == 180) ? TString("") : TString::Format("c%d_%d/", cLow, cHigh);
}

bool ensure_dir(const TString &dir)
{
	if (gSystem->mkdir(dir, true) != 0 && gSystem->AccessPathName(dir)) {
		gSystem->Exec(TString::Format("mkdir -p %s", dir.Data()));
	}
	if (gSystem->AccessPathName(dir)) {
		std::cerr << "ERROR: cannot create output directory: " << dir << std::endl;
		return false;
	}
	return true;
}

bool is_pt_integrated_bin(double ptLow, double ptHigh, double yLow, double yHigh)
{
	const double expectedLow = (yLow >= 1.6 && yHigh <= 2.4) ? 3.5 : 6.5;
	return std::fabs(ptLow - expectedLow) < 1e-6 && std::fabs(ptHigh - 40.0) < 1e-6;
}
...
void draw_ctau_binned_mass(double ptLow = 3.5, double ptHigh = 6.5,
	...
	bool useLogY = false, int cLow = 0, int cHigh = 180)
{
	if (!is_pt_integrated_bin(ptLow, ptHigh, yLow, yHigh) && (cLow != 0 || cHigh != 180)) {
		std::cout << Form("[INFO] centrality subranges are only run for pT-integrated bins; requested cBin %d-%d ignored", cLow, cHigh) << std::endl;
		cLow = 0;
		cHigh = 180;
	}
	...
	const TString cPath = centrality_path(cLow, cHigh);
	const TString pTag = pt_tag(ptLow, ptHigh);
	const TString tag = bin_tag(ptLow, ptHigh, yLow, yHigh, cLow, cHigh);
	const TString outFigDir = TString::Format("figs/%s/%s%s", yTag.Data(), cPath.Data(), pTag.Data());
	const TString outRootDir = TString::Format("roots/%s/%s%s", yTag.Data(), cPath.Data(), pTag.Data());
	if (!ensure_dir(outFigDir) || !ensure_dir(outRootDir))
		return;
	...
	const TString baseCut = Form(
		...
		"(fabs(y) > %.12g && fabs(y) < %.12g) && (cBin >= %d && cBin < %d) && "
		"(recoQQsign==0) && %s && !TMath::IsNaN(ctau3D) && !TMath::IsNaN(ctau3DRes) && "
	...
	std::cout << Form("[INFO] centrality: cBin %d-%d", cLow, cHigh) << std::endl;
