...
std::unique_ptr<RooFitResult> fit_mass_with_retry(RooAbsPdf &pdf, RooDataSet &data, int &attemptUsed,
	const RooArgSet *constraints = nullptr, RooRealVar *bkgMassLambda = nullptr)
{
	auto resetBkgMassLambdaInsideRange = [&]() {
		if (!bkgMassLambda)
			return;
		// A failed low-statistics fit can leave the exponential slope microscopically
		// outside its upper bound.  Newer RooFit versions throw on the next fit
		// instead of silently clipping it, so put it safely inside the range.
		const double safeMin = bkgMassLambda->getMin() + 1e-3;
		const double safeMax = bkgMassLambda->getMax() - 9e-4;
		const double safeValue = std::clamp(bkgMassLambda->getVal(), safeMin, safeMax);
		if (safeValue != bkgMassLambda->getVal())
		{
			std::cout << Form("[INFO] resetting %s from %.17g to %.17g before mass-fit retry",
				bkgMassLambda->GetName(), bkgMassLambda->getVal(), safeValue) << std::endl;
			bkgMassLambda->setVal(safeValue);
		}
	};
	...
	bool drawMassSlices)
{
	...
	// ------------------------------------------------------------------
	auto fitResult = fit_mass_with_retry(mass_pdf, *dataSlice, out.fitAttempt, &massConstraints, &bkg_mass_lambda);
	if (fitResult)
	{
	...
	int cLow = 0, int cHigh = 180)
{
	...
	TString cutBasic = Form("(mass>2.6 && mass<3.5) && (pt > %g && pt < %g) && (fabs(y) > %g && fabs(y) < %g) && (cBin >= %d && cBin < %d) && (recoQQsign==0) && %s",
		ptLow, ptHigh, yLow, yHigh, cLow, cHigh, kAccCut);
	...
	std::cout << Form("[INFO] selected PbPb data entries for raw mass diagnostic: basic=%d final=%d",
	...
	bool massSlicesOnly = false, int cLow = 0, int cHigh = 180)
{
	...
	std::cout << Form("[INFO] selected PbPb data entries: basic=%d final=%d", dataBasic->numEntries(), dataSel->numEntries()) << std::endl;
	...
	if (optimizedCtauBinningApplied && edges.size() >= 2)
	{
		...
		std::cout << Form("[INFO] optimized ctau mass-slice binning for %s: merged %zu -> %zu bins; note: %s",
			bin_tag(ptLow, ptHigh, yLow, yHigh, cLow, cHigh).Data(), beforeMerge, edges.size() - 1, ctauBinningNote) << std::endl;
	}
			...
			for (size_t batchStart = 0; batchStart + 1 < edges.size(); batchStart += static_cast<size_t>(jobs))
			{
				...
				for (size_t i = batchStart; i < batchEnd; ++i)
				{
					...
					const TString expr = TString::Format(
						"subrange_mass(%g,%g,%g,%g,%.17g,%.17g,%.17g,-1,%s,%s,%s,false,%.17g,%d,1,false,%d,true,%d,%d)",
						...
						centerVetoHalfWidth, originalSlice, jobs, cLow, cHigh);
