...
const char *kDataRoot = "/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/skimmedFiles/OniaRooDataSet_miniAOD_isMC0_JPsi_cent0_200_Effw0_Accw0_PtW0_TnP0_230721.root";
//const char *kDataRoot = "/data/Oniatree/skimmedFiles/OniaRooDataSet_miniAOD_isMC0_JPsi_cent0_200_Effw0_Accw0_PtW0_TnP0_230721.root";
const char *kDatasetName = "dataset";
const char *kReferenceDir = "/data/hwan/RAA_subrange_fit/pb_jpsi";
...
TString bin_tag(float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh)
{
	return TString::Format("c%d_%d_y%s_%s_pt%s_%s",
		cLow, cHigh, format_tag(yLow).Data(), format_tag(yHigh).Data(),
...
void subrange_ctau(float ptLow = 3.0, float ptHigh = 6.5, float yLow = 1.6, float yHigh = 2.4,
	...
	bool massSlicesOnly = false, int cLow = 0, int cHigh = 180)
{
	...
	const TString tag = bin_tag(ptLow, ptHigh, yLow, yHigh, cLow, cHigh);
	...
	if (const auto *manualRange = find_manual_ctau_fit_range(ptLow, ptHigh, yLow, yHigh, cLow, cHigh))
	{
	...
	if (optimizedCtauBinningApplied)
	{
		...
		if (monotonicTailMerges > 0)
		{
			std::cout << Form("[INFO] monotonic tail binning for %s: applied %zu outward-width merges",
				bin_tag(ptLow, ptHigh, yLow, yHigh, cLow, cHigh).Data(), monotonicTailMerges) << std::endl;
		}
		if (manualCtauFitRangeApplied)
		{
			actualManualCtauFitLow = ctauFitSlices.front().ctLow;
		...
		std::cout << Form("[INFO] optimized final ctau binning for %s: merged %zu -> %zu bins; note: %s",
			bin_tag(ptLow, ptHigh, yLow, yHigh, cLow, cHigh).Data(), beforeMerge, ctauFitSlices.size(), ctauBinningNote) << std::endl;
	}
	...
	// ------------------------------------------------------------------
	// Fit bFraction directly in the PR/NP shape combination instead of deriving it
	// from floating prompt/nonprompt signal yields after the fit.
