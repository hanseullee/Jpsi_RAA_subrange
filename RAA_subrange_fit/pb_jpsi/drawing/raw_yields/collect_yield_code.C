...
// Fwd
root -l -b -q 'collect_yield_code.C(1.6,2.4,"../../roots","oo_y16to24","NooY16to24",false)'

// Mid
root -l -b -q 'collect_yield_code.C(0.0,1.6,"../../roots","oo_y16","NooY16",false)'
*/ 
...
TString resolveRootsDir(const TString &rootsDir)
{
  ...
  if (rootsDir == "roots")
  {
    const TString fromMovedYieldDir = macroDir() + "/../../roots";
    dir = existingDir(fromMovedYieldDir);
    if (!dir.IsNull())
      return dir;
  }
...
bool readResultFile(const TString &path, BinResult &out)
{
  static const std::regex kNameRe(
      R"((?:fit2d_result|subrange_result)_y([0-9]+\.[0-9]+)_([0-9]+\.[0-9]+)_pt([0-9]+\.[0-9]+)_([0-9]+\.[0-9]+)\.root)");
  ...
  double value = 0.0;
  double error = 0.0;
  bool hasValue = readParam(*f, "Nsig", value);
  bool hasError = readParam(*f, "NsigErr", error);

  if (!(hasValue && hasError))
  {
    double prompt = 0.0;
    double nonprompt = 0.0;
    double promptErr = 0.0;
    double nonpromptErr = 0.0;
    const bool hasPrompt = readParam(*f, "Nprompt", prompt);
    const bool hasNonprompt = readParam(*f, "Nnonprompt", nonprompt);
    const bool hasPromptErr = readParam(*f, "NpromptErr", promptErr);
    const bool hasNonpromptErr = readParam(*f, "NnonpromptErr", nonpromptErr);
    if (hasPrompt && hasNonprompt && hasPromptErr && hasNonpromptErr)
    {
      value = prompt + nonprompt;
      error = std::sqrt(promptErr * promptErr + nonpromptErr * nonpromptErr);
      hasValue = true;
      hasError = true;
    }
  }
  ...
  if (!(hasValue && hasError))
  {
    ...
    if (!modelResult)
      return false;
    auto *var = dynamic_cast<RooRealVar *>(modelResult->floatParsFinal().find("Nsig"));
    if (!var)
      return false;
    value = var->getVal();
    error = var->getError();
...
bool containsAnotherSelectedBin(const BinResult &candidate, const std::vector<BinResult> &selected)
{
  for (const auto &other : selected)
  {
    const bool sameBin = std::abs(candidate.ptLow - other.ptLow) < 1e-9 &&
                         std::abs(candidate.ptHigh - other.ptHigh) < 1e-9;
    if (sameBin)
      continue;
    if (candidate.ptLow <= other.ptLow + 1e-9 && candidate.ptHigh >= other.ptHigh - 1e-9)
      return true;
  }
  return false;
}
...
void collect_yield_code(double yLow = 1.6,
                        double yHigh = 2.4,
                        ...
                        bool showSources = false)
{
  ...
  const TString cmd = Form("find %s -type f \\( -name 'fit2d_result_*.root' -o -name 'subrange_result_*.root' \\)", resolvedRootsDir.Data());
  ...
  selected.erase(std::remove_if(selected.begin(), selected.end(),
                                [&selected](const BinResult &candidate) {
                                  return containsAnotherSelectedBin(candidate, selected);
                                }),
                 selected.end());
  ...
  std::vector<double> ptLowVals, ptHighVals, yieldVals, yieldErrs;
  for (const auto &item : selected)
  {
    ptLowVals.push_back(item.ptLow);
    ptHighVals.push_back(item.ptHigh);
    yieldVals.push_back(item.value);
    yieldErrs.push_back(item.error);
    if (showSources)
    {
  ...
  std::cout << "\n";
  printArray(Form("double val_%s[%s] = ", prefix.Data(), countName.Data()), yieldVals, 5);
  printArray(Form("double stat_%s[%s] = ", prefix.Data(), countName.Data()), yieldErrs, 5);
  std::cout << "\n";
