		...
		)
{
	...
	const TString figTag = cTag + "_" + yTag + "_" + ptTag + "_bkgOff";
	const TString modelTag = cTag + "_" + yTag + "_" + ptTag;
	...
	// load dataset
	// ------------------------------------------------------------------
    TFile* f1 = new TFile("/Volumes/Dropbox/Dropbox/Analysis/HiData/WinUbuntu/Work1/JpsiV2PbPb2020_New/20260815_SubRange/skimmedFiles/OniaRooDataSet_miniAOD_isMC1_JPsi_Prompt_cent0_200_Effw0_Accw0_PtW0_TnP0_240530.root", "read");
    //TFile* f1 = new TFile("/data/Oniatree/skimmedFiles/OniaRooDataSet_miniAOD_isMC1_JPsi_Prompt_cent0_200_Effw0_Accw0_PtW0_TnP0_240530.root", "read");
	...
	RooArgSet fitVars(*(ws->var("ctau3DRes")), *(ws->var("ctau3D")), *(ws->var("ctau3DErr")), *(ws->var("mass")), *(ws->var("pt")), *(ws->var("y")));
	if (ws->var("cBin")) fitVars.add(*(ws->var("cBin")));
	RooDataSet *dsAB = (RooDataSet*)datasetW->reduce(fitVars, kineCut.Data() );
    ...
    if (drawFromSavedFit) {
        ...
        mass_result.reset(mass_pdf->fitTo(*dsAB, Extended(), Save(), Strategy(2), PrintLevel(0), NumCPU(nCPU), Offset(true), SumW2Error(isWeighted), Minimizer("Minuit2", "migrad")));
        if (mass_result && (mass_result->status() != 0 || mass_result->covQual() < 2)) {
            ...
            mass_result.reset(mass_pdf->fitTo(*dsAB, Extended(), Save(), Strategy(2), PrintLevel(0), Offset(true), SumW2Error(isWeighted)));
        }
